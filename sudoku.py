...
lgroups = [[(x,y) for y in xrange(a,a+3) for x in xrange(b,b+3)]
...
def exact_cover(keys, rows, need=81):
    ...
    for row in rs:
        ...
        frows = [r for r in rows if not any((r[ks[0]],r[ks[1]],r[ks[2]],r[ks[3]]))]
        ...
        if len(solution) == need:
            return solution
    # we didn't find a solution containing the number of elements we needed, terminate branch
    return []

class Sudoku:
    def __init__(self,grid):
        self.grid = grid
        self.ngrid = [[set(range(1,10)) for x in xrange(9)] for y in xrange(9)]
        self.ec_matrix = list()
        self.init_ngrid()
        self.init_ec_matrix()
